...
class RGCNBasisLayer(RGCNLayer):
    def __init__(self, inp_dim, out_dim, aggregator, attn_rel_emb_dim, num_rels, num_bases=-1, bias=None, device=0,
        ...
        self.attn_rel_emb = nn.Parameter(torch.Tensor(self.num_rels, self.out_dim))
    ...
    # def propagate(self, g, norm, edge_rel_emd, target_rel_emd_new):
        ...
        def msg_func(edges):
            ...
            edge_rel_emd = self.attn_rel_emb.index_select(0, edges.data['type'])
            target_rel_emd_new = self.attn_rel_emb.index_select(0, edges.data['label'])
